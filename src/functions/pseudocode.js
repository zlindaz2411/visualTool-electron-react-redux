...
export function getPseudocode(name) {
  switch (name) {
    ...
    case Algorithm.BORUVKA: //Borvska's
      ...
      boruvka.push("\xa0 \xa0foreach(component in components)");
    ...
    case Algorithm.PARALLEL: // Parallel Boruvska
      ...
      boruvkaParallel.push("\xa0 \xa0foreach(component in components)");
    ...
    case Algorithm.ESAU: // Esau Wiliams pseudocode
      ...
      esauWilliams.push("\xa0 \xa0foreach (node i in nodeList)");
    ...
    case Algorithm.SIMULATED:
      ...
      simulated.push("\xa0 \xa0 \xa0 \xa0\xa0 if(weight(new T) < weight(T))");
    ...
    case Algorithm.PENALTY:
      ...
      penalty.push("\xa0 \xa0 new T = generateNeighbourhood(T)");
      penalty.push("\xa0 \xa0 if(new T violates degree constraint)");
      ...
      penalty.push(
        "\xa0 \xa0 \xa0 \xa0\xa0 if(penalised weight(new T)  < weight(T) or exp(penalised weight(new T) - weight(T))/Temperature_k > random[0,1]))"
      );
      penalty.push("\xa0 \xa0 \xa0 \xa0\xa0 \xa0\xa0 T = new T");
      penalty.push("\xa0 \xa0 else");
      penalty.push(
        "\xa0 \xa0 \xa0 \xa0\xa0 if(constraint(new T) < constraint(T) or exp(constraint(new T) - constraint(T))/Temperature_k > random[0,1]))"
      );
      penalty.push("\xa0 \xa0 \xa0 \xa0\xa0  \xa0\xa0 T = new T");
