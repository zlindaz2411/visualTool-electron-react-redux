...
export class Graph {
  ...
  removeEdge(edgeIndex) {
    ...
    this.adjacents[this.edges[edgeIndex].source] = this.removeAdjacent(
      source,
      edgeIndex
    );
    let target = this.adjacents[this.edges[edgeIndex].target];
    this.adjacents[this.edges[edgeIndex].target] = this.removeAdjacent(
      target,
      edgeIndex
    );
  ...
  removeAdjacent(source, edgeIndex) {
    for (let i = 0; i < source.length; i++) {
      if (
        source[i].source == this.edges[edgeIndex].source &&
        source[i].target == this.edges[edgeIndex].target
      ) {
        source.splice(i, 1);
        i--;
      } else if (
        source[i].source == this.edges[edgeIndex].target &&
        source[i].target == this.edges[edgeIndex].source
      ) {
        source.splice(i, 1);
        i--;
      }
