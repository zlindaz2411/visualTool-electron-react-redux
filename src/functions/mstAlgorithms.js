...
export async function parallelPromise(graph) {
  try {
    ...
    while (num > 1) {
      ...
      //Let promises to handle the selection of hte cheapest edge(alternative of threads for Javascript)
      let promises = edges.map(async edge => {
        await findCheapest(edge, subset, cheapest);
      });
      let result = await Promise.all(promises);
...
function findCheapest(edge, subset, cheapest) {
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      let v = subset.find(edge.target);
      if (u != v) {
        if (cheapest[u] == -1 || edge.weight < cheapest[u].weight)
          cheapest[u] = edge;
        if (cheapest[v] == -1 || edge.weight < cheapest[v].weight)
          cheapest[v] = edge;
      }
      resolve(edge);
    });
