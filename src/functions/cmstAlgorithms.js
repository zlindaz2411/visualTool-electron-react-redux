  ...
  try {
    ...
    while (CMST.length < len - 1 && edges.length > 0) {
      ...
      if (!uf.connected(source, target)) {
        let componentCapacity = unionSet(components[source], components[target])
          .size;
        ...
        if (c <= capacity) {
          ...
          for (
            let it = components[target].values(), val = null;
            (val = it.next().value);
...
function findRoot(nodes, edges) {
  ...
  for (let i = 0; i < edges.length; i++) {
    if (!nodeConnection.has(edges[i].source)) {
      ...
      nodeConnection.set(
        edges[i].source,
        nodeConnection.get(edges[i].source) + 1
      );
    }
    if (!nodeConnection.has(edges[i].target)) {
      ...
      nodeConnection.set(
        edges[i].target,
        nodeConnection.get(edges[i].target) + 1
      );
    }
  }
  return [...nodeConnection.entries()].reduce((a, e) =>
    e[1] > a[1] ? e : a
  )[0];
}
