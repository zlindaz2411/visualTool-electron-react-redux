...
import {
  ...
  boruvkas,
  parallel
} from "../functions/mstAlgorithms";
import { esauWilliams } from "../functions/cmstAlgorithms";
import {
  kruskalConstrained,
  simulatedAnnealing,
  simulatedAnnealingPenalty
  ...
  for (let i = 0; i < list.length; i++) {
    if (list[i] == Algorithm.KRUSKAL) {
      ...
      if (kruskalConstrained(graph, degree) == ErrMessage.DCMST_NOT_FOUND) {
        ...
        result.push(
          calculateTime(function() {
            kruskalConstrained(graph, degree);
          })
        );
      }
      ...
      if (simulatedAnnealing(graph, degree) == ErrMessage.DCMST_NOT_FOUND) {
        ...
        result.push(
          calculateTime(function() {
            simulatedAnnealing(graph, degree);
          })
        );
      }
    } else if (list[i] == Algorithm.PENALTY) {
      if (
        simulatedAnnealingPenalty(graph, degree) == ErrMessage.DCMST_NOT_FOUND
      ) {
        result.push(-1);
      } else {
        result.push(
          calculateTime(function() {
            simulatedAnnealingPenalty(graph, degree);
          })
        );
      }
    }
