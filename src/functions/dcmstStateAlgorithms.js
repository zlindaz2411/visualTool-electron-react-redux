...
export function simulatedAnnealingPenalty(graph, degree) {
  try {
    ...
    while (K_LEVEL < MAX_TEMP_LEVEL) {
      ...
      if (isViolated) {
        violatedTimes++;
        addStates(states, MST, [], [], violatedTimes, 4);
      } else {
        violatedTimes--;
        addStates(states, MST, [], [], violatedTimes, 5);
      }
      ...
      if (violatedTimes < graph.nodes.length / 2) {
        ...
        if (acceptanceProb < 0 || prob > realNum) {
          ...
          weight = getWeight(MST);
          addStates(states, [], MST, [], "", 11);
        } else {
          MST = oldMST.slice();
        }
      }
      K_LEVEL++;
      addStates(states, MST, [], [], "k_level = " + K_LEVEL, 12);
      addStates(
        states,
        MST,
        [],
