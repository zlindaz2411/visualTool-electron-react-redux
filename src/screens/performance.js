...
import {
  comparePerformanceByTime,
  comparePerformanceByWeight
...
class PerformancePage extends Component {
  ...
  handleClose(capacity) {
    let incorrect = capacity
      ? !validateNumber(this.state.textCapacity) ||
        validateEmpty(this.state.textCapacity)
      : !validateNumber(this.state.textDegree) ||
        validateEmpty(this.state.textDegree);
    if (incorrect) {
      onlyNumberErrorMessage();
    } else {
      if (capacity) {
  ...
  handleCheckboxChange(e) {
    ...
    if (e.target.checked) {
      if (
        name == Algorithm.CONSTRAINED ||
        name == Algorithm.SIMULATED ||
        name == Algorithm.PENALTY
      ) {
        this.setState({
    ...
    return (
      <div className="performance_wrap">
        ...
        <div className="sub_text">
          <h2>
            Select algorithms that you want to compare the performance and click
            "Compare".&ensp;&ensp;
            <span
              title="Note that the parallel algorithm is implemented in parallel, due to the software problem (being single threaded), its performance is sequential"
              className="tooltip"
            >
              &#9432;
            </span>
...
export default withRouter(connect(mapStateToProps, {})(PerformancePage));
